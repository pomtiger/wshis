...
{    //ปีกกาแรก */
  ...
  mysqli_close($conn);
    ...
    if($result){
      echo "<script>";
      echo "alert('Update Profile Succesfully:แก้ข้อมูลสำเร็จแย้ว');";
      echo "window.location ='index.php'; "; //เด้งไปหน้า
      //echo "window.location = 'index.php?act=edit_profile'; ";   //กลับไปหน้าแก้ไขเดิม
      //echo "window.location = 'index.php?ID=$m_id&act=edit'; ";  //กลับไปหน้าแก้ไขเดิม
      echo "</script>";
    } 
    else {      
      echo "<script>";
      echo "alert('ERROR PROFILE EDIT!:ไม่สำเร็จจ๊ะ ว้ายๆ');";
